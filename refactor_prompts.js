...
const extractFunctionBody = code.match(/export function buildGenerationPrompt[\s\S]*?\{\s*return\s*`([\s\S]*?)`;\n}/);

if (!extractFunctionBody) {
  console.log("Could not find buildGenerationPrompt body");
  process.exit(1);
}

// We need to replace the entire buildGenerationPrompt with a new one that composes the constants
